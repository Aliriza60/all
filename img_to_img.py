...
new_size = (ap.width,ap.height)  

image_org = image_org.resize(new_size)


if ap.single_file_or_not == 1:

    pipeline = StableDiffusionImg2ImgPipeline.from_single_file(ap.model_path, safety_checker=None).to("cuda")

elif ap.single_file_or_not == 2:

    pipeline = StableDiffusionImg2ImgPipeline.from_pretrained(ap.model_name, safety_checker=None,use_safetensors=True).to("cuda")
